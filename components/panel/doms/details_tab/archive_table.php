...
<table class="w-full" data-aos="fade-in" data-aos-delay="100">
                    ...
                    if (!isset($_GET['page_id']) or $_GET['page_id']==""){
                        ...
                        $sql = "SELECT logs.id, users.name, users.sur_name, logs.when, logs.object_id, logs.object_type, log_types.type,logs.description FROM `logs` left join users on users.id=logs.user_id join log_types on log_types.id=logs.type where logs.object_type = 'products_doms' and object_id='$product_id' order by id desc limit 20 offset ".($page_id - 1) * 20;
