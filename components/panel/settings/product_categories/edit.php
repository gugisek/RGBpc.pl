...
$sql = "SELECT id, category, parent_id from product_categories where id = $id";
...
<form action="scripts/settings/products/categories/edit.php" method="POST">
    ...
    <div class="-mt-4">
        <h1 class="text-md font-bold">Edytuj kategorię</h1>
        <p class=" text-xs text-gray-500">Edytujesz kategorię: <?=$category?></p>
    </div>
    ...
    <div class="w-full">
        ...
        <div class="mt-2">
            <input name="name" id="name" type="text" value="<?=$category?>" placeholder="Wpisz nazwę" class="border rounded-xl w-full py-1.5 px-4 text-sm border-gray-400 focus:ring-0 focus:outline-0 focus:bg-[#1c1c1c] focus:border-[#1c1c1c] focus:shadow-xl duration-150 font-medium focus:text-white" required>
        </div>
    </div>
    </div>
    ...
    <div class="mt-4 flex flex-row gap-2">
        <div class="w-full">
            ...
            <div class="mt-2">
                <select name="parent_id" id="parent_id" type="text" placeholder="Wybierz status" class=" cursor-pointer border rounded-xl py-1.5 w-full px-4 text-sm border-gray-400 focus:ring-0 focus:outline-0 focus:bg-[#1c1c1c] focus:border-[#1c1c1c] focus:shadow-xl duration-150 font-medium focus:text-white" required>
    ...
    <div class="mt-6 sm:mt-6 mb-2 sm:flex sm:flex-row-reverse">
        ...
        <button onclick="popupCategoriesCloseConfirm()" type="button" class="active:scale-95 mt-3 inline-flex w-full justify-center rounded-full px-4 py-2 text-sm font-medium text-gray-900 shadow-sm ring-inset ring-1 ring-[#3d3d3d] hover:ring-gray-500 hover:bg-gray-500 hover:text-white hover:shadow-xl duration-150 sm:mt-0 sm:w-auto">Nie zapisuj</button>
